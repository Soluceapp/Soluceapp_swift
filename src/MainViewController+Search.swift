...
private extension MainViewController {
    ...
    func loadSearch() {
        ...
        provider.get(query) { (movie, tv, people, articles) in
            ...
            let buttonConfigs = [
                StackButtonConfiguration(label: "Activité", value: articles?.count, kind: .news, showCount: false),
                StackButtonConfiguration(label: "Activité", value: movie?.total_results, kind: .movie),
                StackButtonConfiguration(label: "Activité", value: tv?.total_results, kind: .tv),
                StackButtonConfiguration(label: "Activité", value: people?.total_results, kind: .people)
            ]
